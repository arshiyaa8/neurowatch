...
class AIService {
  ...
  async processChat({ patientId = 1, message, currentScanResult = null }) {
    return new Promise((resolve) => {
      db.get('SELECT * FROM patients WHERE id = ?', [patientId], (err, patient) => {
        ...
        // FAST / Stroke Keyword evaluation
        const strokeKeywords = ['numbness', 'weakness', 'slurred', 'face drooping', 'arm drift', 'vision loss', 'headache', 'dizziness', 'paralysis', 'stroke', 'fast'];
        ...
        if (matchedKeywords.length >= 2 || msgLower.includes('severe headache') || msgLower.includes('cannot speak') || (currentScanResult && currentScanResult.faceAsymmetry)) {
          ...
          reply = `🚨 **CRITICAL STROKE RISK DETECTED**\nBased on your reported symptoms ("${matchedKeywords.join(', ')}") and past history of ${patient.medical_history}, there is an **88% probability of acute stroke**.\n\n⚡ **Actions initiated immediately:**\n1. Calling Indian Emergency Ambulance (108).\n2. Alerting your saved emergency contacts (Loved Ones).\n3. Alerting nearby hospitals & bystanders.\n\n**First-Aid Instructions:** Keep calm, sit or lie down with head slightly elevated (30 degrees). Do NOT take food, water, or aspirin until evaluated by emergency doctors. Note the exact time symptoms started.`;
        } else if (matchedKeywords.length === 1 || msgLower.includes('dizzy') || msgLower.includes('headache')) {
          urgency = 'MODERATE';
          strokeProbability = 45;
          reply = `⚠️ **Moderate Neurological Risk Identified**\nYou mentioned "${matchedKeywords.join(', ')}". Given your medical profile (${patient.medical_history}), we recommend completing an instant BE-FAST camera screen or consulting a neurologist immediately.\n\nIf symptoms worsen or face drooping/arm weakness develops, tap the **Emergency SOS 108** button immediately.`;
        } else if (msgLower.includes('history') || msgLower.includes('record') || msgLower.includes('document')) {
          urgency = 'LOW';
          strokeProbability = 10;
          reply = `📋 **Medical Profile & History Summary for ${patient.name}:**\n- **Age**: ${patient.age}\n- **Blood Group**: ${patient.blood_group || 'B+'}\n- **Known Conditions**: ${patient.medical_history}\n- **Current Medications**: ${patient.medications}\n- **Allergies**: ${patient.allergies || 'None'}\n\nAll your uploaded MRI, CT scan, and ECG documents are securely stored and instantly accessible by emergency hospitals during an SOS event.`;
        } else {
          urgency = 'LOW';
          strokeProbability = 12;
          reply = `Hello ${patient.name}. I am your NeuroWatch AI Health Assistant. I continuously monitor your BE-FAST risk indicators alongside your past medical history.\n\nHow can I help you today? You can report symptoms, ask about your saved medical records, or request a FAST bystander scan.`;
        }
  ...
  async analyzeDocument({ fileName, fileType, docType }) {
    ...
    if (typeUpper.includes('MRI')) {
      summary = 'Brain MRI Report: Ischemic changes detected in periventricular white matter. No acute intracranial hemorrhage. Recommend regular blood pressure tracking & neurologist follow-up.';
